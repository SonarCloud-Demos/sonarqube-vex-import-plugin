...
`VexImportPageDefinition` always registers the page — SonarQube's page registry is built once at startup with no supported way to add or remove a page afterward, so conditionally skipping registration here would make toggling the setting require a restart before the tab appeared or disappeared, which is exactly what this design avoids. Instead, `veximport.enabled` is enforced entirely at runtime by `pluginSettings.ts` (see the `VexImportWizard` entry below): the tab stays visible in the menu regardless of the setting, but its content is a disabled notice instead of the wizard when off. That makes toggling **instantaneous** — no restart, ever — at the cost of the tab itself never disappearing from the menu. No admin page beyond the auto-generated settings form — the boolean is simple enough not to need one. The Java layer otherwise only registers the page; all logic runs in the browser (and, separately, in the companion Python CLI — see §6).
...
| 12 | No undo, and branches aren't a safety net | Applied status changes cannot be rolled back — SonarQube exposes no "restore previous status" API. Importing on a side branch first is *not* a substitute for that: SonarQube's own issue/risk branch-sync only carries a *new* finding's attributes forward on merge — a status change made on a branch for a risk that already existed on main before that branch was created does not propagate back to main on merge (confirmed both in SonarQube's own docs and, for SCA specifically, in several 2026-09 internal bug reports showing risk-status branch inheritance is currently unreliable). `StepIntro` recommends previewing an import against a throwaway, never-merged branch instead — analyzed independently first, since a new branch doesn't inherit main's detected risks for free. |

### Toolchain

Dev/build/test dependencies (`package.json` devDependencies, `pom.xml` test-scope deps) are kept CVE-free by tracking upstream advisories and bumping regularly — not a one-time pass. `esbuild` was briefly held at `0.19.2` because newer versions dropped support for downleveling destructuring against `conf/esbuild-config.js`'s `TARGET_BROWSERS` — that list (`chrome58`/`firefox57`/`safari11`/`edge18`) was a leftover from the original plugin-starter scaffold, not a real requirement of this plugin. Modernized to `chrome100`/`firefox100`/`safari15`/`edge100` and `esbuild` bumped to latest accordingly. Runtime dependencies (React 16.14.0, the SonarQube UI toolkit) are pinned to what the host SonarQube version's plugin API supports and are out of scope for CVE-driven bumps.
