    ...
    .then((result) => {
      ...
      http
        .createServer((req, res) => {
          if (req.url.match(/static\/veximport/)) {
            // We need to remove the plugin "sub-directory". This is not part of the build output,
            // but is dynamically added by SonarQube. We build: `vex_import.js`, but
            // SonarQube serves this under `/static/veximport/vex_import.js`.
            req.url = req.url.replace('static/veximport/', '');
